...
$buscar_vehicle = "SELECT * FROM vehicle WHERE matricula = '" . $db->real_escape_string($matricula) ."'";
$elimina_vehicle = "DELETE FROM vehicle WHERE matricula = '" . $db->real_escape_string($matricula) ."'";
...
try{
    if(!$db->query($elimina_vehicle)){
        echo 'Query error: ' . $db->error;
        die();
    }
    else{
        echo "<br>Numero de files esborrades de la base de dades: " . $db->affected_rows . "<br>";
    }
}
catch(Exception $e){
...
echo "<a type='button' href='cerca.html' class='btn btn-outline-primary'>Cerca</a>";
